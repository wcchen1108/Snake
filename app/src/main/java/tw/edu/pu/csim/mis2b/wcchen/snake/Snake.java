...
public class Snake {
    ...
    public Snake(Bitmap bm, int x, int y, int length) {
        ...
        this.y = y;
        this.length = length;
        bm_body_bottom_left = Bitmap.createBitmap(bm, 0, 0, GameView.size0fMap, GameView.size0fMap);
        bm_body_bottom_right = Bitmap.createBitmap(bm, GameView.size0fMap, 0, GameView.size0fMap, GameView.size0fMap);
        bm_body_horizontal = Bitmap.createBitmap(bm, 2*GameView.size0fMap, 0, GameView.size0fMap, GameView.size0fMap);
        bm_body_top_left = Bitmap.createBitmap(bm, 3*GameView.size0fMap, 0, GameView.size0fMap, GameView.size0fMap);
        bm_body_top_right = Bitmap.createBitmap(bm, 4*GameView.size0fMap, 0, GameView.size0fMap, GameView.size0fMap);
        bm_body_vertical = Bitmap.createBitmap(bm, 5*GameView.size0fMap, 0, GameView.size0fMap, GameView.size0fMap);
        bm_head_down = Bitmap.createBitmap(bm, 6*GameView.size0fMap, 0, GameView.size0fMap, GameView.size0fMap);
        bm_head_left = Bitmap.createBitmap(bm, 7*GameView.size0fMap, 0, GameView.size0fMap, GameView.size0fMap);
        bm_head_right = Bitmap.createBitmap(bm, 8*GameView.size0fMap, 0, GameView.size0fMap, GameView.size0fMap);
        bm_head_up = Bitmap.createBitmap(bm, 9*GameView.size0fMap, 0, GameView.size0fMap, GameView.size0fMap);
        bm_tail_up = Bitmap.createBitmap(bm, 10*GameView.size0fMap, 0, GameView.size0fMap, GameView.size0fMap);
        bm_tail_right = Bitmap.createBitmap(bm, 11*GameView.size0fMap, 0, GameView.size0fMap, GameView.size0fMap);
        bm_tail_left = Bitmap.createBitmap(bm, 12*GameView.size0fMap, 0, GameView.size0fMap, GameView.size0fMap);
        bm_tail_down = Bitmap.createBitmap(bm, 13*GameView.size0fMap, 0, GameView.size0fMap, GameView.size0fMap);
        arrPartSnake.add(new PartSnake(bm_head_right, x, y));       //Pic:Snake_head
        for (int i = 1; i < length -1; i++){
            arrPartSnake.add(new PartSnake(bm_body_horizontal, arrPartSnake.get(i-1).getX() - GameView.size0fMap, y));      //Pic:Snake_body
        }
        arrPartSnake.add(new PartSnake(bm_tail_right, arrPartSnake.get(length-2).getX() - GameView.size0fMap, y));      //Pic:Snake_tail
        setMove_right(true);
    }

    public void update() {       //Pic:Update
        for (int i = length - 1; i > 0; i--) {
            arrPartSnake.get(i).setX(arrPartSnake.get(i - 1).getX());
            arrPartSnake.get(i).setY(arrPartSnake.get(i - 1).getY());
        }
        if (move_right) {       //移動時的X,Y分別增加、減少
            arrPartSnake.get(0).setX(arrPartSnake.get(0).getX() + GameView.size0fMap);
            arrPartSnake.get(0).setBm(bm_head_right);
        } else if (move_left) {
            arrPartSnake.get(0).setX(arrPartSnake.get(0).getX() - GameView.size0fMap);
            arrPartSnake.get(0).setBm(bm_head_left);
        } else if (move_top) {
            arrPartSnake.get(0).setY(arrPartSnake.get(0).getY() - GameView.size0fMap);
    ...
    public void addPart() {
        ...
        if (p.getBm() == bm_tail_right){
            this.arrPartSnake.add(new PartSnake(bm_tail_right, p.getX() - GameView.size0fMap, p.getY()));
        }else if (p.getBm() == bm_tail_left){
            this.arrPartSnake.add(new PartSnake(bm_tail_left, p.getX() + GameView.size0fMap, p.getY()));
        }else if (p.getBm() == bm_tail_up){
            this.arrPartSnake.add(new PartSnake(bm_tail_up, p.getX(), p.getY() + GameView.size0fMap));
        }else if (p.getBm() == bm_tail_down){
            this.arrPartSnake.add(new PartSnake(bm_tail_down, p.getX(), p.getY() - GameView.size0fMap));
        }
    }
}
